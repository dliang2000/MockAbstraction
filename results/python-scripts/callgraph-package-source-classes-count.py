...
with open('CallGraphEdgePackageSourceClassCounts.csv', mode='w') as outfile:
    writer = csv.writer(outfile)
    header = ['Benchmark', 'basic-only (Source)', 'CI (Source)', 'CIPP (Source)', '1-object-sens (Source)']
            ...
            if os.path.isfile(curr_file):
                with open(curr_file, newline='') as csvfile:
                    ...
                    for row in reader:
                        ...
                        clazz = source_method.replace("<", "").split(":")[0]
                        if clazz not in clazz_set and benchmarkRootPackageDict[benchmark] in clazz:
                            clazz_set.add(clazz)
