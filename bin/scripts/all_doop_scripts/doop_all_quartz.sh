...
for base_analysis in basic-only context-insensitive context-insensitive-plusplus; do
  ...
  for n in NORMAL NO_INTERPROC; do
    ...
    echo /usr/bin/time -o $HOME/souffle-$base_analysis-$BENCHMARK-$n souffle -F results/$RESULT/$base_analysis/java_8/quartz-core-$base_analysis-$n/ -M $n souffle-logic/analyses/mocks/mocks-after.dl -D results/$RESULT/$base_analysis/java_8/quartz-core-$base_analysis-$n/
