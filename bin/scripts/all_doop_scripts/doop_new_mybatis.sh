...
for n in NORMAL NO_INTERPROC; do
  ...
  cp -a last-analysis results/$RESULT/basic-only/java_8/mybatis-basic-new-$n

  /usr/bin/time -o $HOME/souffle-basic-$BENCHMARK-$n souffle -F results/$RESULT/basic-only/java_8/mybatis-basic-new-$n -M $n souffle-logic/analyses/mocks/mocks-after.dl -D results/$RESULT/basic-only/java_8/mybatis-basic-new-$n/
...
for n in NORMAL NO_INTERPROC; do
  ...
  cp -a last-analysis results/$RESULT/context-insensitive/java_8/mybatis-new-$n

  /usr/bin/time -o $HOME/souffle-context-insensitive-$BENCHMARK-$n souffle -F results/$RESULT/context-insensitive/java_8/mybatis-new-$n -M $n souffle-logic/analyses/mocks/mocks-after.dl -D results/$RESULT/context-insensitive/java_8/mybatis-new-$n/
