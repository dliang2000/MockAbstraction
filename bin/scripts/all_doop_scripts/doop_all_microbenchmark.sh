...
#for base_analysis in context-insensitive ; do
  ...
  echo ./doop -a $base_analysis -i $MACHINE_SPECIFIC_PATH/Benchmarks/$BENCHMARK/target/$RESULT.jar -i $MACHINE_SPECIFIC_PATH/Benchmarks/$BENCHMARK/target/$RESULT-tests.jar -i $MACHINE_SPECIFIC_PATH/Benchmarks/$BENCHMARK/mvn_dependencies --id microbenchmark-$base_analysis-NORMAL --souffle-jobs 32 --main ca.liang.RootDriver --define-cpp-macro $USE_WHAT --extra-logic souffle-logic/analyses/mocks/mocks.dl &>> $MACHINE_SPECIFIC_PATH/results/microbenchmark-results/microbenchmark-$base_analysis-NORMAL.log
  ./doop -a $base_analysis -i $MACHINE_SPECIFIC_PATH/Benchmarks/$BENCHMARK/target/$RESULT.jar -i $MACHINE_SPECIFIC_PATH/Benchmarks/$BENCHMARK/target/$RESULT-tests.jar -i $MACHINE_SPECIFIC_PATH/Benchmarks/$BENCHMARK/mvn_dependencies --id microbenchmark-$base_analysis-NORMAL --souffle-jobs 32 --main ca.liang.RootDriver --define-cpp-macro $USE_WHAT --extra-logic souffle-logic/analyses/mocks/mocks.dl &>> $MACHINE_SPECIFIC_PATH/results/microbenchmark-results/microbenchmark-$base_analysis-NORMAL.log
